...
class MEDDLY::taggedbdd : public evmdd_forest {
  public:
  ...
  public:
    taggedbdd(int dsl, domain *d, const policies &p, int* level_reduction_rule=NULL);
    ~taggedbdd();
...
//    virtual bool isOneSuppressed(const unpacked_node &nb) const;
    ...
    virtual enumerator::iterator* makeFullIter() const {
      return new taggedbdd_iterator(this);
    }
  ...
  public:
    void createReducedNode(int in, MEDDLY::unpacked_node *un, long& tag,
        node_handle& node) {
      MEDDLY_DCASSERT(un);
      normalize(*un, tag);
      //un->computeHash();
      createReducedHelper(in, *un, tag, node);
  ...
  protected:
    class taggedbdd_iterator : public enumerator::iterator {
      public:
        taggedbdd_iterator(const expert_forest* F);
        virtual ~taggedbdd_iterator();

        virtual bool start(const dd_edge &e);
        virtual bool next();
      private:
        bool first(int k, long tag, node_handle p);
    };
};
