...
class MEDDLY::mt_mxd_int : public mtmxd_forest {
  public:
    ...
    mt_mxd_int(unsigned dsl, domain *d, const policies &p, int* level_reduction_rule=NULL, int tv=0);
    ~mt_mxd_int();

    void createEdge(long val, dd_edge &e);
    void createEdge(const int_extra* const* vlist, const int_extra* const* vplist, const long* terms, int N, dd_edge& e);
    /// deprecated
    ...
    void evaluate(const dd_edge& f, const int_extra* vlist, const int_extra* vplist,
        long &term) const;
    /// deprecated
