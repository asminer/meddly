...
#ifdef HAVE_LIBGMP
 class mpzDensityClass {
 public:
     MEDDLY::mpz_object density;
     int index;
     mpzDensityClass(){index=0; density.setValue(0);/* isIn=false;neverIn=false;*/}
     explicit mpzDensityClass(MEDDLY::mpz_object _density,int _index):
     density(_density),index(_index){ }
     mpzDensityClass(const mpzDensityClass &t)
              {
                         density=t.density;
                         index=t.index;
              }
     mpzDensityClass& operator=(const mpzDensityClass& that) {
                  if (this == &that)
                     return *this;
     ...
     bool operator < (const mpzDensityClass& t) const
              {
                MEDDLY::mpz_object dmpz;
                  return (dmpz.compare(density,t.density)==-1); // density<t.density
              }
     virtual ~mpzDensityClass(){}
  };
#endif
...
private:
      ...
      int maxid=0;
      void uniqueNodesforp(node_handle p,std::list<int> &result );
      void getNC(int lvl, node_handle a,bool*visitedNode,std::set<int> &result);
      void uniqueAboveNodesforp(node_handle p,std::list<int> &result );
      int RemoveDuplicateSet(int lvl, bool* levelarray,std::list<int>* uniquelist, /*std::set<int>levels,*/std::map<int,int>map,dd_edge &e/*,std::set<int>&RNA,std::set<int>&RNB*/);
