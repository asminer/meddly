  ...
  class ct_typebased : public compute_table {
    ...
    private:  // helper methods
      ...
      inline unsigned hash(const int* k, int length) const {
        return raw_hash(k, length) % table.size();
      }
...
: compute_table(s, op, slot)
{
  ...
  */
  ...
  mstats.incMemUsed(table.size() * sizeof(int));
  mstats.incMemAlloc(table.size() * sizeof(int));
...
::findEntry(ct_entry_key* key)
{
  ...
  int* answer = 0;
  int* preventry = 0;
  unsigned hcurr = key->getHash() % table.size();
...
void MEDDLY::ct_typebased<MONOLITHIC, CHAINED>::addEntry(ct_entry_key* key, const ct_entry_result &res)
{
  ...
  if (CHAINED) {
    if (newsize != table.size()) {
      ...
      MEDDLY_DCASSERT(newsize > table.size());
      mstats.incMemUsed( (newsize - table.size()) * sizeof(int) );
      mstats.incMemAlloc( (newsize - table.size()) * sizeof(int) );

      table.resize(newsize, 0);
    ...
    if (newsize != table.size()) {
      ...
      */
      ...
      std::vector <int> oldT(newsize, 0);
      table.swap(oldT);

      mstats.incMemUsed(newsize * sizeof(int));
      mstats.incMemAlloc(newsize * sizeof(int));

      rehashTable(oldT.data(), oldT.size());
      // free(oldT);

      mstats.decMemUsed(oldT.size() * sizeof(int));
      mstats.decMemAlloc(oldT.size() * sizeof(int));
  ...
  if (CHAINED) {
    ...
    if (perf.numEntries < tableShrink) {
      ...
      */
      ...
      MEDDLY_DCASSERT(table.size() > newsize);
      mstats.decMemUsed( (table.size() - newsize) * sizeof(int) );
      mstats.decMemAlloc( (table.size() - newsize) * sizeof(int) );
...
#endif
            if (item.isStaleEntry(*entry, mark)) {
                return YES_stale();
            }
            entry++;
        } else {
