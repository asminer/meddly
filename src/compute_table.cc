...
MEDDLY::compute_table::~compute_table()
{
  ...
  for (cache_entry* curr = nodes; curr != end; ++curr)
  {
    if (!isFreeNode(*curr)) {
      DCASSERT(curr->dataOffset != -1);
      curr->owner->discardEntry(getDataAddress(*curr));
    }
...
{
  nodes[h].owner->showEntry(s, getDataAddress(nodes[h]));
}
...
void MEDDLY::compute_table::removeStales(operation* owner)
{
  static bool removingStales = false;
  if (!removingStales) {
    DCASSERT(ht != 0 || fsht != 0);
      ...
      if (ht) {
        ...
        for (cache_entry* current = nodes; current != end; ++current)
        {
          ...
          if (current->owner == owner &&
              owner->isEntryStale(getDataAddress(*current)))
...
void MEDDLY::compute_table::removeEntries(operation* owner)
{
...
MEDDLY::createMonolithicTable(const settings &s)
{
  compute_table* ct = new compute_table;
  ct->setPolicy(s.doComputeTablesUseChaining, s.maxComputeTableSize);
  return ct;
}
