...
#ifdef DEVELOPMENT_CODE
            E.xferNode(_down.at(n));
            _edge.at(n) = E.getEdgeValue();
#else
            E.xferNode(_down[n]);
...
#ifdef DEVELOPMENT_CODE
            _down.at(n) = h;
            _edge.at(n).set(the_edge_type, p);
#else
            _down[n] = h;
