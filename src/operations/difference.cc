...
bool MEDDLY::diffr_mdd::checkTerminals(int a, int b, int& c)
{
  if (a == 0 || b == -1) {
    c = 0;
    return true;
...
bool MEDDLY::diffr_mxd::checkTerminals(int a, int b, int& c)
{
  if (a == 0) {
    c = 0;
    return true;
...
MEDDLY::binary_opname* MEDDLY::initializeDifference(const settings &s)
{
