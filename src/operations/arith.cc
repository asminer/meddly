...
// *                                                                *
// *                                                                *
// *      Massive template operation for multi-terminal arith.      *
// *                                                                *
// *                                                                *
// ******************************************************************
...
namespace MEDDLY {
    ...
    class arith_op_mt : public binary_operation {
        public:
            arith_op_mt(forest* arg1, forest* arg2, forest* res);
            virtual ~arith_op_mt();
...
template <class ATYPE>
MEDDLY::arith_op_mt<ATYPE>::arith_op_mt(forest* arg1, forest* arg2,
...
template <class ATYPE>
MEDDLY::arith_op_mt<ATYPE>::~arith_op_mt()
{
    ct->markForDestroy();
}

template <class ATYPE>
void MEDDLY::arith_op_mt<ATYPE>::compute(int L, unsigned in,
        const edge_value &av, node_handle ap,
        const edge_value &bv, node_handle bp,
        edge_value &cv, node_handle &cp)
{
...
template <class ATYPE>
void MEDDLY::arith_op_mt<ATYPE>::_compute(int L, unsigned in,
...
MEDDLY::binary_operation* MEDDLY::MAXIMUM(forest* a, forest* b, forest* c)
{
    ...
    if (c->getEdgeLabeling() == edge_labeling::MULTI_TERMINAL) {
        ...
        if (use_reals) {
            bop = new arith_op_mt<mt_maximum<float> > (a,b,c);
        } else {
            bop = new arith_op_mt<mt_maximum<long> > (a,b,c);
        }
...
MEDDLY::binary_operation* MEDDLY::MINIMUM(forest* a, forest* b, forest* c)
{
    ...
    if (c->getEdgeLabeling() == edge_labeling::MULTI_TERMINAL) {
        ...
        if (use_reals) {
            bop = new arith_op_mt<mt_minimum<float> > (a,b,c);
        } else {
            bop = new arith_op_mt<mt_minimum<long> > (a,b,c);
        }
