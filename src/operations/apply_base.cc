...
MEDDLY::generic_binary_mdd::compute_ext(node_handle a, node_handle b)
{
  ...
  unpacked_node *A = (aLevel < resultLevel)
    ...
    : unpacked_node::newFromNode(arg1F, a, SPARSE_ONLY)
    ;
  ...
  unpacked_node *B = (bLevel < resultLevel)
    ...
    : unpacked_node::newFromNode(arg2F, b, SPARSE_ONLY)
    ;
...
::compute_r(int in, int resultLevel, node_handle a, node_handle b)
{
  ...
  if (aLevel == resultLevel) {
    ...
    A->initRedundant(resultLevel, a);
  } else {
    A->initIdentity(resultLevel, in, a);
    canSaveResult = false;
  }
  ...
  if (bLevel == resultLevel) {
    ...
    B->initRedundant(resultLevel, b);
  } else {
    B->initIdentity(resultLevel, in, b);
    canSaveResult = false;
  }
...
::compute_r(int in, int level, long aev, node_handle a, long bev, node_handle b, long& cev, node_handle &c)
{
  ...
  unpacked_node* C =
    unpacked_node::newWritable(resF, level, resultSize, FULL_ONLY);
  ...
  if (aLevel == level) {
    ...
    A->initRedundant(level, 0L, a);
  } else {
    A->initIdentity(level, in, 0L, a);
  }

  if (bLevel == level) {
    B->initFromNode(b);
  } else if (arg2F->isFullyReduced()) {
    B->initRedundant(level, 0L, b);
  } else {
    B->initIdentity(level, in, 0L, b);
  }
  ...
  float& cev, node_handle& c)
{
  ...
  if (aLevel == resultLevel) {
    ...
    A->initRedundant(resultLevel, 1.0f, a);
  } else {
    A->initIdentity(resultLevel, in, 1.0f, a);
  }

  if (bLevel == resultLevel) {
    B->initFromNode(b);
  } else if (arg2F->isFullyReduced()) {
    B->initRedundant(resultLevel, 1.0f, b);
  } else {
    B->initIdentity(resultLevel, in, 1.0f, b);
  }
