...
namespace MEDDLY {
  ...
  class otfbfs_by_events_opname;
  class otfbfs_by_events_op;

  class common_otf_dfs_by_events_mt;
  class common_otf_bfs_by_events_mt;

  class forwd_otf_dfs_by_events_mt;
  class forwd_otf_bfs_by_events_mt;
  class bckwd_otf_dfs_by_events_mt;

  class fb_otf_saturation_opname;
  class fb_otf_bfs_opname;
...
// ******************************************************************
// *                                                                *
// *                    otfbfs_by_events_opname  class              *
// *                                                                *
// ******************************************************************

/** Simple class to keep compute table happy.
*/
class MEDDLY::otfbfs_by_events_opname : public unary_opname {
  static otfbfs_by_events_opname* instance;
  public:
    otfbfs_by_events_opname();

    static otfbfs_by_events_opname* getInstance();

};

MEDDLY::otfbfs_by_events_opname* MEDDLY::otfbfs_by_events_opname::instance = 0;

MEDDLY::otfbfs_by_events_opname::otfbfs_by_events_opname()
 : unary_opname("Otf_Bfs_by_events")
{
}

MEDDLY::otfbfs_by_events_opname* MEDDLY::otfbfs_by_events_opname::getInstance()
{
  if (0==instance) instance = new otfbfs_by_events_opname;
  return instance;
}
...
// ******************************************************************
// *                                                                *
// *                      otfbfs_by_events_op  class                *
// *                                                                *
// ******************************************************************

class MEDDLY::otfbfs_by_events_op : public unary_operation {
    common_otf_bfs_by_events_mt* parent;
  public:
    otfbfs_by_events_op(common_otf_bfs_by_events_mt* p,
      expert_forest* argF, expert_forest* resF);
    virtual ~otfbfs_by_events_op();

    void RSBFSfly(const dd_edge& in, dd_edge& out, satotf_opname::otf_relation* rel);
...
// ******************************************************************
// *                                                                *
// *            common_otf_bfs_by_events_mt  class                  *
// *                                                                *
// ******************************************************************

class MEDDLY::common_otf_bfs_by_events_mt : public specialized_operation {
  public:
    common_otf_bfs_by_events_mt(satotf_opname* opcode,
      satotf_opname::otf_relation* rel);
    virtual ~common_otf_bfs_by_events_mt();
...
// ******************************************************************
// *                                                                *
// *           otfbfs_by_events_op  methods                     *
// *                                                                *
// ******************************************************************

MEDDLY::otfbfs_by_events_op
::otfbfs_by_events_op(common_otf_bfs_by_events_mt* p,
  expert_forest* argF, expert_forest* resF)
  : unary_operation(otfbfs_by_events_opname::getInstance(), 1, argF, resF)
{
  parent = p;

  const char* name = otfbfs_by_events_opname::getInstance()->getName();
  ct_entry_type* et;
...
MEDDLY::otfbfs_by_events_op::~otfbfs_by_events_op()
{
  removeAllComputeTableEntries();
}


void MEDDLY::otfbfs_by_events_op::RSBFSfly(const dd_edge& init, dd_edge& reachableStates, satotf_opname::otf_relation* rel)
{
...
// ******************************************************************
// *                                                                *
// *           common_otf_bfs_by_events_mt  methods                     *
// *                                                                *
// ******************************************************************

MEDDLY::common_otf_bfs_by_events_mt::common_otf_bfs_by_events_mt(
  satotf_opname* opcode,
  satotf_opname::otf_relation* relation)
: specialized_operation(opcode, 1)
{
...
void MEDDLY::common_otf_bfs_by_events_mt
::compute(const dd_edge &a, dd_edge &c)
{
  // Initialize operations
  mddUnion = getOperation(UNION, resF, resF, resF);
  ...
  // Execute saturation operation
  otfbfs_by_events_op* so = new otfbfs_by_events_op(this, arg1F, resF);
  so->RSBFSfly(a,c,rel);
...
// ******************************************************************
// *       common_otf_bfs_by_events_mt::indexq  methods                 *
// ******************************************************************

MEDDLY::common_otf_bfs_by_events_mt::indexq::indexq()
{
  data = 0;
  size = 0;
  head = NULPTR;
...
void MEDDLY::common_otf_bfs_by_events_mt::indexq::resize(unsigned sz)
{
  if (sz <= size) return;
  data = (int*) realloc(data, sz * sizeof(int));
  if (0==data)
...
// ******************************************************************
// *       common_otf_bfs_by_events_mt::charbuf methods             *
// ******************************************************************

MEDDLY::common_otf_bfs_by_events_mt::charbuf::charbuf()
{
  data = 0;
  size = 0;
}
...
void MEDDLY::common_otf_bfs_by_events_mt::charbuf::resize(unsigned sz)
{
  if (sz <= size) return;
  data = (char*) realloc(data, sz * sizeof(char));
  if (0==data)
...
// ******************************************************************
// *                                                                *
// *             forwd_otf_bfs_by_events_mt class                   *
// *                                                                *
// ******************************************************************

class MEDDLY::forwd_otf_bfs_by_events_mt : public common_otf_bfs_by_events_mt {
  public:
    forwd_otf_bfs_by_events_mt(satotf_opname* opcode,
    satotf_opname::otf_relation* rel);
  protected:
    // virtual void saturateHelper(unpacked_node& mdd);
    // node_handle recFire(node_handle mdd, node_handle mxd);
...
MEDDLY::forwd_otf_bfs_by_events_mt::forwd_otf_bfs_by_events_mt(
  satotf_opname* opcode,
  satotf_opname::otf_relation* rel)
  : common_otf_bfs_by_events_mt(opcode, rel)
{
}


// void MEDDLY::forwd_otf_bfs_by_events_mt::saturateHelper(unpacked_node& nb)
// {
//   const int level = nb.getLevel();
//   const int nEventsAtThisLevel = rel->getNumOfEvents(level);
//   if (0 == nEventsAtThisLevel) return;
...
//
// // Same as post-image, except we saturate before reducing.
// MEDDLY::node_handle MEDDLY::forwd_otf_bfs_by_events_mt::recFire(
//   node_handle mdd, node_handle mxd)
// {
//   //      Node builder expansion
//   //          - done
...
//
//
// void MEDDLY::forwd_otf_bfs_by_events_mt::recFireHelper(
//   const unsigned i,
//   const int rLevel,
//   const node_handle Ru_i,
//   const node_handle A_i,
...
class MEDDLY::fb_otf_bfs_opname : public satotf_opname {
    bool forward;
  public:
    fb_otf_bfs_opname(bool fwd);
    virtual specialized_operation* buildOperation(arguments* a);
};

MEDDLY::fb_otf_bfs_opname::fb_otf_bfs_opname(bool fwd)
 : satotf_opname(fwd ? "OtfBfsFwd" : "OtfBfsBack")
{
  forward = fwd;
}

MEDDLY::specialized_operation*
MEDDLY::fb_otf_bfs_opname::buildOperation(arguments* a)
{
  otf_relation* rel = dynamic_cast<otf_relation*>(a);
  if (0==rel) throw error(error::INVALID_ARGUMENT, __FILE__, __LINE__);
  ...
  MEDDLY::specialized_operation* op = 0;
  if (forward)
    op = new forwd_otf_bfs_by_events_mt(this, rel);
  else {
    throw error(error::NOT_IMPLEMENTED, __FILE__, __LINE__);
    // op = new bckwd_otf_dfs_by_events_mt(this, rel);
  }
...
MEDDLY::satotf_opname* MEDDLY::initOtfBfsForward()
{
  return new fb_otf_bfs_opname(true);
}
