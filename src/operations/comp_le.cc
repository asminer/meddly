...
class lessequal_mdd : public generic_binary_mdd {
  public:
    lessequal_mdd(binary_list& opcode, forest* arg1,
...
class lessequal_mxd : public generic_binbylevel_mxd {
  public:
    lessequal_mxd(binary_list& opcode, forest* arg1,
      forest* arg2, forest* res)
...
MEDDLY::lessequal_opname::buildOperation(binary_list &c, forest* a1, forest* a2,
  forest* r)
{
  ...
  if (r->getEdgeLabeling() == edge_labeling::MULTI_TERMINAL) {
    if (use_reals) {
      if (r->isForRelations())
        return new lessequal_mxd<float>(c, a1, a2, r);
      else
        return new lessequal_mdd<float>(c, a1, a2, r);
    } else {
      if (r->isForRelations())
        return new lessequal_mxd<int>(c, a1, a2, r);
      else
        return new lessequal_mdd<int>(c, a1, a2, r);
    }
  }
