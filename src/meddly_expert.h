...
namespace MEDDLY {
  ...
  // Generic relation node for marking dependent transitions
...
/** Pieces of an all-purpose implicit relation.
 
 Each piece has its set of inputs, free-variables and outputs.
 It has two functions, namely delta and omega.
 delta() specifies the next local state of the node at the current level given the value of inputs, guessed values of free variables and current state.
 omega() specifies the set of outputs that are to be used by the next piece in the relation.
 
 delta(), omega() are virtual methods.
 equals() : to detect when two nodes actually represent the same function.
...
private:
  ...
  int* output_vars;
  int* input_vars;
  int* dep_vars; // because not all input variables are to be used by this piece, some are passed on.
  int* free_vars;
  int noof_outputs;
  int noof_inputs;
  int noof_deps;
